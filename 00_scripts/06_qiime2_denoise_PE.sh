...
#NEG_CONTROL=/scratch_vol0/fungi/BioIndic_La_Reunion_Island_Lagoon_II/98_database_files/Negative_control_Sample_RepSeq_V4.qza
NEG_CONTROL=/scratch_vol0/fungi/BioIndic_La_Reunion_Island_Lagoon_II/99_contamination
TMPDIR=/scratch_vol0
...
# Paste them in a contamination_seq.fasta file, then :
 ...
 qiime tools import \
  ...
  --output-path $NEG_CONTROL/contamination_seq_16S.qza \
  --type 'FeatureData[Sequence]'

qiime quality-control exclude-seqs --i-query-sequences core/RepSeq.qza \
      					     --i-reference-sequences $NEG_CONTROL/contamination_seq_16S.qza \
      					     --p-method vsearch \
      					     --p-threads 6 \
...
# Paste them in a contamination_seq.fasta file, then :
 ...
 qiime tools import \
  ...
  --output-path $NEG_CONTROL/contamination_seq_18S.qza \
  --type 'FeatureData[Sequence]'

qiime quality-control exclude-seqs --i-query-sequences core/RepSeq.qza \
      					     --i-reference-sequences $NEG_CONTROL/contamination_seq_18S.qza \
      					     --p-method vsearch \
      					     --p-threads 6 \
      					     --p-perc-identity 1.00 \
      					     --p-perc-query-aligned 1.00 \
...
# Paste them in a contamination_seq.fasta file, then :
 ...
 qiime tools import \
  ...
  --output-path $NEG_CONTROL/contamination_seq_ITS.qza \
  --type 'FeatureData[Sequence]'

qiime quality-control exclude-seqs --i-query-sequences core/RepSeq.qza \
      					     --i-reference-sequences $NEG_CONTROL/contamination_seq_ITS.qza \
      					     --p-method vsearch \
      					     --p-threads 6 \
      					     --p-perc-identity 1.00 \
      					     --p-perc-query-aligned 1.00 \
